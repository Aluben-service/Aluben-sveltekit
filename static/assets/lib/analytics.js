		...
		var $t,
			...
			Ht = function (t, e) {
				...
				qt &&
					...
					tt("t") &&
						(n.duration = Math.round((et() - gt - Rt) / 1e3)),
					(Rt = 0),
					(gt = et()),
					tt("scro") && (n.scrolled = Math.max(0, vt, Tt())),
					e || !g.sendBeacon
						? pt(n, h, f)
						: g.sendBeacon(k + "/append", S(n)));
			};
		M(
			"visibilitychange",
			function () {
		...
		var Ut,
			...
			Bt = function (t) {
				...
				if (
					...
					return (
						"hash" == yt && s.hash && (e += s.hash.split("?")[0]), e
					);
				p(b + "ignoring " + e);
			},
			Nt = function (t, e, n) {
				...
				if (r && $t != r) {
					($t = r),
						...
						tt("v") &&
							...
							(Dt.viewport_height =
								Math.max(q[H] || 0, l.innerHeight || 0) ||
								null)),
						tt("l") && g[A] && (Dt[A] = g[A]),
						N &&
							tt("sc") &&
							((Dt.screen_width = N.width),
