		...
		b = class extends p.EventEmitter {
			constructor(t = __uv$config) {
				...
				(this.address = i[~~(Math.random() * i.length)]),
					(this.bareClient = new p.BareClient());
			}
			async fetch({ request: t }) {
				...
				try {
					...
					(e.meta.origin = location.origin),
						(e.meta.base = e.meta.url =
							new URL(e.sourceUrl(t.url)));
					let r = new g(
						t,
						this,
						e,
						w.includes(t.method.toUpperCase())
							? null
							: await t.blob(),
					);
					if (
					...
					if ((this.emit("request", f), f.intercepted))
					...
					let h = await this.bareClient.fetch(i, {
						...
						o = new y(r, h),
						d = new m(o, null, null);
					if ((this.emit("beforemod", d), d.intercepted))
						return d.returnValue;
					for (let n of x) o.headers[n] && delete o.headers[n];
					if (
						(o.headers.location &&
						...
						let n = o.headers["content-disposition"];
						if (
							!/\s*?((inline|attachment);\s*?)filename=/i.test(n)
						) {
							let u = /^\s*?attachment/i.test(n)
									? "attachment"
									: "inline",
								[v] = new URL(h.finalURL).pathname
									.split("/")
									.slice(-1);
							o.headers["content-disposition"] =
							...
							case "worker":
								{
									let n = [
										...
										.join(",");
									(o.body = `if (!self.__uv && self.importScripts) { ${e.createJsInject(this.address, this.bareClient.manfiest, e.cookie.serialize(l, e.meta, !0), t.referrer)} importScripts(${n}); }
`),
										(o.body += e.js.rewrite(
											await h.text(),
										));
								...
								break;
							case "style":
								o.body = e.rewriteCSS(await h.text());
								break;
							case "iframe":
							case "document":
								S(
									...
									(o.body = e.rewriteHtml(await h.text(), {
										document: !0,
	...
	function S(s, t = "") {
		return (
			(p.mime.contentType(t || s.pathname) || "text/html").split(
				";",
			)[0] === "text/html"
		);
	}
	...
	function C(s, t) {
		let i = new URL(s),
			e = `remoteHostname.textContent = ${JSON.stringify(i.hostname)};bareServer.href = ${JSON.stringify(t)};uvHostname.textContent = ${JSON.stringify(location.hostname)};reload.addEventListener("click", () => location.reload());uvVersion.textContent = ${JSON.stringify("3.0.0")};`;
	...
	function U(s, t, i, e, a, r, l) {
		if (e === "The specified host could not be resolved.") return C(r, l);
		let c =
			`errorTitle.textContent = ${JSON.stringify(s)};errorCode.textContent = ${JSON.stringify(t)};` +
			(i ? `errorId.textContent = ${JSON.stringify(i)};` : "") +
			`errorMessage.textContent =  ${JSON.stringify(e)};errorTrace.value = ${JSON.stringify(a)};fetchedURL.textContent = ${JSON.stringify(r)};bareServer.href = ${JSON.stringify(l)};for (const node of document.querySelectorAll("#uvHostname")) node.textContent = ${JSON.stringify(location.hostname)};reload.addEventListener("click", () => location.reload());uvVersion.textContent = ${JSON.stringify("3.0.0")};`;
		return (
	...
	function E(s) {
		return s instanceof Error && typeof s.body == "object";
	}
	function O(s, t, i) {
		let e,
			a,
		...
		return (
			E(s)
				...
				: ((e = 500),
					...
					(r = s instanceof Error ? s.name : "UNKNOWN")),
			new Response(U(a, r, l, c, String(s), t, i), {
				status: e,
